'use strict';
/*
 * 版本號散在四個地方（index.html 與 leads/index.html 的 ?v=、app.js 的 APP_VERSION、version.json），
 * 任何一處對不上都會出事，而且不會讓任何功能測試變紅：
 *   - version.json 比程式新 → 更新提示無限迴圈
 *   - index.html 的 ?v= 沒跟著換 → 使用者拿到快取的舊 js
 * 這支只做一件事：四處一致，而且兩個頁面裡自己的 js／css 每一個都帶了版本號。
 */
...
test('version.json、APP_VERSION、index.html 的 ?v= 三處一致', () => {
  ...
  for (const page of ['index.html', 'leads/index.html']) {
    const versions = new Set([...read(page).matchAll(/\?v=([\w-]+)/g)].map((m) => m[1]));
    assert.deepEqual([...versions], [json], `${page} 裡的 ?v= 有不一樣的：${[...versions].join('、')}`);
  }
});

test('index.html 與 leads/index.html 裡自己的 js／css 每一個都帶版本號', () => {
  for (const [page, min] of [['index.html', 10], ['leads/index.html', 2]]) {
